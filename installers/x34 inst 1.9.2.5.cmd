...
@rem 
 ...
 :alreyinst
 echo The %x34% is alredy installed on version %x34sytemversion% do you want to delete it?
 set /p alreyinst-yes-no=(Y/N)
 set alredyiiii=true
 if "%alreyinst-yes-no%" == "Y" goto unistall
 if "%alreyinst-yes-no%" == "N" exit
 if "%alreyinst-yes-no%" == "y" goto unistall
 if "%alreyinst-yes-no%" == "n" exit
